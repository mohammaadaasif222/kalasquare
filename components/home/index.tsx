...
//   const { banners, loading, error, getBanners,  } = useBanner();
...
export default function HomePageComponent() {
  ...
  const carousels = banners?.filter((banner) => banner.type === "TOP")
  const cardBanners = banners?.filter((banner) => banner.type === "CARD")
  const bottomBanners = banners?.filter((banner) => banner.type === "BOTTOM")

  return (
    <main className="pb-20 md:pb-8">
      <BannerSection banners={carousels} loading={loading} title="" showControls={true} />
      ...
      <div className="max-w-6xl m-auto">
        ...
        <section className="py-16 md:py-16 px-4">
          <div className="w-full max-w-7xl mx-auto">
            ...
            {/* View All Button */}
            <div className="flex justify-end px-4">
              <button className=" hover:underline text-[var(--brand)] font-normal">View All</button>
            </div>
          </div>
        </section>
